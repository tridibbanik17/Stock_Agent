...
- **Supabase** stores only email, tickers, schedule, `last_sent_at`, daily send counters, and `unsubscribe_token` (not holdings, not grades).
- **Subscribe** is email-keyed upsert: Save & Subscribe overwrites that address’s watchlist and schedule. If the saved schedule is already inside the early/overdue window, the API **sends that report before responding** and returns `report_send_status` of `sent` or `failed` (not an optimistic queue).
...
- **Gemini** is optional BYOK in the Chrome extension only. Quote refresh does **not** call Gemini. An optional **Explain grades** button can send one batched prompt (metrics + notes + headlines) from the device to Google. Numeric grades, cron, and email stay rules-only.
